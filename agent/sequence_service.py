...
class SequenceService:
    def __init__(self, *, wled: WLEDClient, looks: LookService, ddp: DDPStreamer, data_dir: str) -> None:
        self.wled = wled
        self.looks = looks
        self.ddp = ddp
        ...
        self._status = SequenceStatus(running=False, file=None, started_at=None, step_index=0, steps_total=0, loop=False)
    ...
    def generate(
        ...
        *,
        ...
        steps_n = max(1, int(duration_s) // step_s)

        steps: List[Dict[str, Any]] = []
        for i in range(steps_n):
            # Insert a DDP step occasionally
            if include_ddp and ddp_patterns and (i % max(10, int(30/step_s)) == 0) and i != 0:
                pat = rng.choice(ddp_patterns)
                params = {}
                # randomize a few known params
                if pat in ("candy_spiral",):
                    params = {"speed": rng.choice([0.3, 0.5, 0.8]), "stripes": rng.choice([6, 8, 10]), "twist": rng.choice([0.8, 1.2, 1.6])}
                elif pat in ("rainbow_cycle", "glitter_rainbow"):
                    params = {"speed": rng.choice([0.04, 0.06, 0.08]), "spread": rng.choice([1.0, 1.3, 2.0])}
                elif pat in ("comet",):
                    params = {"speed": rng.choice([150.0, 220.0, 320.0]), "tail": rng.choice([40, 70, 120])}
                elif pat in ("snowfall",):
                    params = {"density": rng.choice([0.004, 0.008, 0.015]), "speed": rng.choice([0.1, 0.15, 0.25])}
                elif pat in ("matrix_rain",):
                    params = {"streaks_per_run": rng.choice([1, 2, 3]), "seed": rng.randrange(1, 999999)}
                steps.append({"type": "ddp", "pattern": pat, "params": params, "duration_s": step_s})
            else:
                look = rng.choice(looks)
                steps.append({"type": "look", "look": look, "duration_s": step_s})
                ...
                while not self._stop.is_set():
                    for i, step in enumerate(steps):
                        ...
                        if typ == "look":
                            ...
                            self.looks.apply_look(look, brightness_override=int(bri) if bri is not None else None)
                            time.sleep(dur)
                        elif typ == "preset":
                            self.wled.set_preset(int(step.get("preset_id", 1)), verbose=False)
                            time.sleep(dur)
                        elif typ == "ddp":
                            pat = str(step.get("pattern"))
                            params = step.get("params") or {}
                            self.ddp.start(pattern=pat, params=params, duration_s=dur, brightness=int(step.get("brightness", 128)), fps=float(step.get("fps", self.ddp.fps_default)))
                            time.sleep(dur)
                            self.ddp.stop()
                        else:
                            time.sleep(dur)
