...
def _norm_dir(val: str | None, default: str = "ccw") -> str:
    ...
    if s in ("cw", "clockwise"):
        return "cw"
    if s in ("ccw", "counterclockwise", "counter-clockwise", "anticlockwise", "anti-clockwise"):
        return "ccw"
    return default
...
class Settings:
    ...
    quad_order_from_street: str  # 'cw' or 'ccw' meaning increasing segment order as seen from the street
...
def load_settings() -> Settings:
    controller_kind = _as_str(os.environ.get("CONTROLLER_KIND"), default="wled").strip().lower() or "wled"

    wled_tree_url = os.environ.get("WLED_TREE_URL", "").strip()
    if controller_kind == "wled" and not wled_tree_url:
        raise RuntimeError("WLED_TREE_URL is required when CONTROLLER_KIND=wled (e.g. http://10.0.0.50)")
    ...
    wled_command_cooldown_ms = max(0, _as_int(os.environ.get("WLED_COMMAND_COOLDOWN_MS"), 250))
    ...
    quad_order_from_street = _norm_dir(os.environ.get("QUAD_ORDER_FROM_STREET"), default="ccw")
    quad_default_start_pos = _norm_pos(os.environ.get("QUAD_DEFAULT_START_POS"), default="front")
    ...
    ddp_destination_id = max(1, min(255, _as_int(os.environ.get("DDP_DESTINATION_ID"), 1)))
    ddp_max_pixels_per_packet = max(1, _as_int(os.environ.get("DDP_MAX_PIXELS_PER_PACKET"), 480))
    ...
    pixel_protocol = _as_str(os.environ.get("PIXEL_PROTOCOL"), default="e131").strip().lower() or "e131"
    ...
    pixel_universe_start = max(0, _as_int(os.environ.get("PIXEL_UNIVERSE_START"), pixel_universe_start_default))
    pixel_channels_per_universe = max(1, min(512, _as_int(os.environ.get("PIXEL_CHANNELS_PER_UNIVERSE"), 510)))
    ...
    pixel_source_name = _as_str(os.environ.get("PIXEL_SOURCE_NAME"), default=agent_name)[:64]
    ...
    auth_jwt_issuer = _as_str(os.environ.get("AUTH_JWT_ISSUER"), default="wled-show-agent")
    ...
    if controller_kind == "pixel":
        if not pixel_host:
            raise RuntimeError("PIXEL_HOST is required when CONTROLLER_KIND=pixel (e.g. 10.0.0.60)")
        if pixel_count <= 0:
            raise RuntimeError("PIXEL_COUNT is required when CONTROLLER_KIND=pixel (e.g. 50)")
        if pixel_protocol not in ("e131", "artnet"):
            raise RuntimeError("PIXEL_PROTOCOL must be 'e131' or 'artnet' when CONTROLLER_KIND=pixel")
